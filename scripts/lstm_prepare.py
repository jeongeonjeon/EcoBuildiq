"""
lstm_prepare.py  (minimal patch to store building_id per window)

- 기존 로직은 유지
- 추가(패치):
  * 각 윈도우(타깃 시점)에 해당하는 building_id의 카테고리 코드 저장
    - data/processed/meta_building_{meter}.npy  (int32)
  * 라벨 복원용 카테고리 목록 저장
    - models/building_id_categories.json
"""
    ...
    for meter in meters:
        ...
        for (src, site, bid, mtype), g in grouped:
            ...
            for i in range(0, len(g) - window_size, STRIDE):
                ...
                # 타깃 시점 building 코드 저장
                b_meta[out_idx] = int(b_codes[i + window_size])

                out_idx += 1

        print(f"[{meter}] wrote {out_idx} windows → {X_path.name}, {y_path.name}, {B_META_PATH.name}")
